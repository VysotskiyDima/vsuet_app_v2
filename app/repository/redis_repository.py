"""Слой доступа к Redis с двумя логическими БД (активная/фоновая).

- endpoints читают из активной БД;
- планировщик пишет в фоновую БД, по завершении цикла меняет их ролями;
- указатель активной БД хранится в отдельной служебной БД (переживает swap).

Чтение по префиксу — через SCAN MATCH + MGET (не KEYS, чтобы не блокировать Redis).
"""
